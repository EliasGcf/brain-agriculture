...
const schema = z.object({
  ...
  totalArea: z.custom<Area>((value) => value instanceof Area).refine((area) => area.value > 0, { message: 'Total area must be positive' }),
  ...
  if (props.arableArea.value + props.vegetationArea.value > props.totalArea.value) {
    ctx.addIssue({ code: 'custom', message: 'Allocated areas cannot exceed total area', path: ['arableArea'] })
  }
})
...
export class Farm extends Entity<Schema> {
  ...
  get producerId() { return this.props.producerId }
  get city() { return this.props.city }
  get state() { return this.props.state }
  get totalArea() { return this.props.totalArea }
  get arableArea() { return this.props.arableArea }
  get vegetationArea() { return this.props.vegetationArea }
}
