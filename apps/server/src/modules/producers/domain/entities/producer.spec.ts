...
describe('Producer', () => {
  it('should be able to create a producer with a valid name and document', () => {
    const producer = Producer.create({ name: 'Maria Silva', document: Document.create('529.982.247-25') })
  ...
  it('should be able to preserve a supplied ID', () => {
    ...
    expect(Producer.create({ name: 'Maria Silva', document: Document.create('52998224725') }, id).id).toBe(id)
  })
})
