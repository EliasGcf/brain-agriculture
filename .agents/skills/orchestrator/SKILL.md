...
The root and reviewer use Luna; routine execution subagents use Luna.
...
The root keeps the Pro profile configuration from `.codex/config.toml`: GPT-5.6 Luna at max reasoning. The role files in `.codex/agents/` explicitly set Luna reasoning to `medium` and reviewer Astra reasoning to `low`. Preserve those efforts when spawning agents unless the user requests a change. Do not change the root model from within a session.
